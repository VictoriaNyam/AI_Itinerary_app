...
def view_itinerary_map(itinerary_id):
    ...
    valid_pois = [
        poi for poi in itinerary.pois
        if poi.original_latitude is not None and poi.original_longitude is not None
    ]
    ...
    max_day = max(poi.day for poi in valid_pois)

    # Initialize map without fixed center
    m = folium.Map()

    # Add markers
    for poi in valid_pois:
        day_idx = (poi.day - 1) % len(colors)
        popup_html = f"""
        ...
        folium.Marker(
            ...
            icon=folium.Icon(color=colors[day_idx], icon='info-sign')
        ).add_to(m)

    # Auto-fit map to all valid POI locations
    poi_coords = [
        [poi.original_latitude, poi.original_longitude] for poi in valid_pois
    ]
    m.fit_bounds(poi_coords)
